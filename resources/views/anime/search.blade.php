    ...
    <div class="xl:grid xl:grid-cols-[1fr_260px] xl:gap-8">
        <section>
            ...
            <div class="mt-8 flex flex-wrap items-center justify-center gap-2">
                ...
                @endphp
                <a href="{{ route('anime.search', array_merge($pageParams, ['page' => max(1, $currentPage - 1)])) }}"
                   class="px-3 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 {{ $currentPage <= 1 ? 'pointer-events-none opacity-40' : '' }}">
                    Prev
                </a>
                @for($p = $startPage; $p <= $endPage; $p++)
                <a href="{{ route('anime.search', array_merge($pageParams, ['page' => $p])) }}"
                   class="px-3 py-2 rounded-lg {{ $p === $currentPage ? 'bg-purple-600 text-white' : 'bg-gray-800 hover:bg-gray-700' }}">
                    {{ $p }}
                </a>
                @endfor
                <a href="{{ route('anime.search', array_merge($pageParams, ['page' => min($lastPage, $currentPage + 1)])) }}"
        ...
        <aside class="mt-8 xl:mt-0 xl:sticky xl:top-24 self-start">
            <div class="mb-4">
                <a href="{{ route('anime.search', ['q' => $query]) }}" class="inline-block px-3 py-2 rounded-lg bg-gray-700 hover:bg-gray-600 text-sm">
                    Reset filters
                </a>
            </div>
            <h2 class="text-sm uppercase tracking-wide text-gray-400 mb-3">Status</h2>
            <div class="grid grid-cols-2 gap-2 mb-4">
                <a href="{{ route('anime.search', array_merge(request()->except(['status', 'page']), ['q' => $query, 'status' => 'airing'])) }}"
                ...
                <a href="{{ route('anime.search', array_merge(request()->except(['status', 'page']), ['q' => $query])) }}"
            ...
            <h2 class="text-sm uppercase tracking-wide text-gray-400 mb-3">Season</h2>
            <div class="grid grid-cols-2 gap-2 mb-4">
                <a href="{{ route('anime.search', array_merge(request()->except(['season', 'page']), ['q' => $query, 'season' => 'spring_2026'])) }}"
                   class="px-3 py-2 rounded-full text-sm text-center {{ ($filters['season'] ?? null) === 'spring_2026' ? 'bg-white text-gray-900' : 'bg-gray-800 text-gray-200 hover:bg-gray-700' }}">
                    Spring '26
                </a>
                <a href="{{ route('anime.search', array_merge(request()->except(['season', 'page']), ['q' => $query])) }}"
                   class="px-3 py-2 rounded-full text-sm text-center {{ empty($filters['season']) ? 'bg-white text-gray-900' : 'bg-gray-800 text-gray-200 hover:bg-gray-700' }}">
                    Any
                </a>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 mb-4">
                <a href="{{ route('anime.search', array_merge(request()->except(['sort', 'page']), ['q' => $query, 'sort' => 'popular'])) }}"
                ...
                <a href="{{ route('anime.search', array_merge(request()->except(['sort', 'page']), ['q' => $query, 'sort' => 'recent'])) }}"
            ...
            <div class="grid grid-cols-2 gap-2 mb-4">
                <a href="{{ route('anime.search', array_merge(request()->except(['type', 'page']), ['q' => $query, 'type' => 'shows'])) }}"
                ...
                <a href="{{ route('anime.search', array_merge(request()->except(['type', 'page']), ['q' => $query, 'type' => 'movies'])) }}"
            ...
            <div class="grid grid-cols-2 gap-2">
                <a href="{{ route('anime.search', array_merge(request()->except(['genre', 'page']), ['q' => $query])) }}"
                ...
                <a href="{{ route('anime.search', array_merge(request()->except(['genre', 'page']), ['q' => $query, 'genre' => $genre])) }}"
